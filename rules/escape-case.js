...
const create = context => {
	return {
		Literal(node) {
			let matches;
			if (typeof node.value === 'string' && (matches = node.raw.match(escapeWithLowercase)) && matches[4].slice(1).match(hasLowercaseCharacter)) {
				context.report({
					node,
					message,
		...
		TemplateElement(node) {
			let matches;
			if (typeof node.value.raw === 'string' && (matches = node.value.raw.match(escapeWithLowercase)) && matches[4].slice(1).match(hasLowercaseCharacter)) {
				context.report({
					node,
					message,
